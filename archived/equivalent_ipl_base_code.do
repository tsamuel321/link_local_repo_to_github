...
*global data "${dir}/data/poverty_query/2.10"

global data "${dir}/data/poverty_query"

*Create a folder to save poverty estimates
*mkdir "${data}"
...
preserve
	...
	forvalues row=1/`r(N)' {
		...
		save `"${data}/`ccc'_urb.dta"', replace
		}
...
*Query rural poverty lines and poverty rates for China, India, and Indonesia.
		...
		forvalues row=1/`r(N)' {
			...
			save `"${data}/`ccc'_rur.dta"', replace
...
forvalues row=1/`r(N)' {
	...
	save `"${data}/`ccc'.dta"', replace
	}

	
*Combine (append) poverty data from all countries
use `"${data}/AGO.dta"', clear 
...
#delimit cr;  
 ...
 foreach x of local ccc {
	append using `"${data}/`x'.dta"', force
	}
	
save `"${data}/ipl2017_poverty_query.dta"', replace	
	


//////Now poverty analysis/////////
use `"${data}/ipl2017_poverty_query.dta"', clear
...
save `"${data}/ipl2017_poverty_query_new.dta"', replace	


*Calculate global poverty rate
use `"${data}/ipl2017_poverty_query_new.dta"', clear
